...
export default function AdminEnrollments() {
  ...
  const loadData = async () => {
    try {
      const [c, s] = await Promise.all([courseAPI.listAll(), authAPI.getStudents()])
      setCourses(c.data)
      setStudents(s.data)
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">Öğrenci Atamaları</h1>

      <div className="grid lg:grid-cols-3 gap-6">
        <div className="bg-white rounded-xl shadow-sm border p-4">
          <h2 className="font-semibold mb-4 flex items-center gap-2"><BookOpen size={20} /> Dersler</h2>
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {courses.map(course => (
              <button key={course.id} onClick={() => setSelectedCourse(course)}
                className={`w-full text-left p-3 rounded-lg border transition ${selectedCourse?.id === course.id ? 'border-blue-500 bg-blue-50' : 'hover:border-gray-300'}`}>
